...
> **贯穿项目**：functions_demo/
> **代码位置**：`app/core/functions.py`
> **测试验证**：`cd functions_demo && uv run pytest -k TestFunctionsBasics -v`
...
这两个函数展示了**单一职责**原则——`letter_grade` 只负责等级转换，`calculate_average` 只负责平均分计算。
...
递归的核心是**基线条件**（`len(items) <= 1`）和**递归步骤**（拆分后调用自身）。
