...
> **贯穿项目**：functions_demo/
> **代码位置**：`app/core/functions.py` (create_student)
> **测试验证**：`cd functions_demo && uv run pytest -k TestFunctionArgs -v`
...
| `comment: str = "", rank: int | None = None` | keyword-only 参数 | 位于 `*args` 后面的参数只能通过关键字传入，语义清晰不易误传 |
