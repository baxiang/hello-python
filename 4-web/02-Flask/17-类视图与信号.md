...
Flask 类视图（Class-Based Views）通过 `View` 基类和 `MethodView`（HTTP 方法自动分发）实现结构化代码。`as_view()` 类方法将类转换为 WSGI 兼容的视图函数，每次请求创建新实例避免状态污染。信号系统基于 Blinker 库实现发布-订阅模式（Pub-Sub），核心信号包括：`request_started`、`request_finished`、`got_request_exception`、`template_rendered`。信号接收器使用弱引用防止 GC 阻塞。
...
### 5.5 RESTful 模式与 MethodView 混用

在复杂业务中，MethodView 可以结合装饰器、mixins 和自定义分发实现灵活的路由：

```python
# methodview_restful_patterns.py
"""MethodView 的 RESTful 高级模式"""
from flask import Flask, request, jsonify, abort
from flask.views import MethodView
from functools import wraps
...
class RESTResource(MethodView):
    ...
    def get(self, resource_id: int | None = None) -> Any:
        """GET 请求 — 列表或详情"""
        if resource_id is not None:
            item: dict[str, Any] | None = self.get_item(resource_id)
            if item is None:
                abort(404, description=f"{self.resource_name} not found")
            return jsonify(item), 200

        # 列表查询
        filters: dict[str, str] = self._extract_filters()
        sort_by: str | None = self._extract_sort()
        page: int = request.args.get("page", 1, type=int)
        per_page: int = request.args.get("per_page", 20, type=int)

        items: list[dict[str, Any]] = self.list_items(filters, sort_by, page, per_page)
        return jsonify({"items": items, "page": page, "per_page": per_page}), 200
    ...
    def post(self) -> tuple[dict[str, Any], int]:
        ...
        errors: dict[str, list[str]] = self.validate_create(data)
        if errors:
            return jsonify({"error": "Validation failed", "fields": errors}), 422

        item: dict[str, Any] = self.create_item(data)
        return jsonify(item), 201

    def put(self, resource_id: int) -> Any:
        """PUT 请求 — 完整更新"""
        data: dict[str, Any] | None = request.get_json()
        if not data:
            abort(400, description="Request body required")

        item: dict[str, Any] | None = self.update_item(resource_id, data)
        if item is None:
            abort(404, description=f"{self.resource_name} not found")
        return jsonify(item), 200

    def patch(self, resource_id: int) -> Any:
        """PATCH 请求 — 部分更新（回退到 PUT 逻辑）"""
        return self.put(resource_id)

    def delete(self, resource_id: int) -> tuple[str, int]:
        """DELETE 请求 — 删除"""
        if not self.delete_item(resource_id):
            abort(404, description=f"{self.resource_name} not found")
        return "", 204

    # ── 子类需实现的方法 ──

    def get_item(self, resource_id: int) -> dict[str, Any] | None:
        raise NotImplementedError

    def list_items(
        self, filters: dict[str, str], sort_by: str | None,
        page: int, per_page: int,
    ) -> list[dict[str, Any]]:
        raise NotImplementedError

    def validate_create(self, data: dict[str, Any]) -> dict[str, list[str]]:
        return {}

    def create_item(self, data: dict[str, Any]) -> dict[str, Any]:
        raise NotImplementedError

    def update_item(self, resource_id: int, data: dict[str, Any]) -> dict[str, Any] | None:
        raise NotImplementedError

    def delete_item(self, resource_id: int) -> bool:
        raise NotImplementedError

    # ── 私有方法 ──

    def _extract_filters(self) -> dict[str, str]:
        filters: dict[str, str] = {}
        for key in self.allowed_filters:
            value: str | None = request.args.get(key)
            if value is not None:
                filters[key] = value
        return filters

    def _extract_sort(self) -> str | None:
        sort_by: str | None = request.args.get("sort_by")
        if sort_by and sort_by in self.allowed_sort_fields:
            return sort_by
        return None
```

### 5.6 MethodView 的 dispatch 机制深入

```python
# methodview_dispatch_deep.py
"""MethodView dispatch 的边界情况与技巧"""
from flask import Flask, request, abort
from flask.views import MethodView
from typing import Any, ClassVar


class AdvancedMethodView(MethodView):
    """扩展 MethodView — 支持中间件和预处理"""

    # 类级别 decorators（所有方法共享）
    decorators: ClassVar[list[Callable]] = []

    # 按方法级别的 decorators
    method_decorators: ClassVar[dict[str, list[Callable]]] = {}

    def dispatch_request(self, *args: Any, **kwargs: Any) -> Any:
        """重写 dispatch 增加请求前/后处理"""
        # 请求前处理
        self.before_request(*args, **kwargs)

        # 调用原生分发
        try:
            response: Any = super().dispatch_request(*args, **kwargs)
    ...
    def after_request(self, response: Any, *args: Any, **kwargs: Any) -> Any:
        """请求后的钩子 — 子类可覆盖"""
        return response

    def on_error(self, error: Exception, *args: Any, **kwargs: Any) -> None:
        """异常钩子 — 子类可覆盖"""
        pass

    @classmethod
    def as_view(cls, name: str, **initkwargs: Any) -> Callable:
        """增强的 as_view — 支持按方法装饰器"""
        view: Callable = super().as_view(name, **initkwargs)

        # 允许方法级装饰器
        for method_name, decorators in cls.method_decorators.items():
            # 暂存原始方法引用
            pass
...
# 使用示例：带请求校验的 API 视图
class ProtectedAPIView(AdvancedMethodView):
    """需要认证的 API 基类"""
    ...
    def on_error(self, error: Exception, *args: Any, **kwargs: Any) -> None:
        ...
        current_app.logger.error(
            "API Error in %s: %s", self.__class__.__name__, error,
        )
```
...
```python
# signal_decoupled_architecture.py
"""使用信号实现解耦的微服务式架构"""
from flask import Flask, g, request
from blinker import Namespace, Signal
from typing import Any
...
# 业务事件信号
order_created: Signal = signals.signal("order-created")
order_paid: Signal = signals.signal("order-paid")
order_shipped: Signal = signals.signal("order-shipped")
user_registered: Signal = signals.signal("user-registered")
file_uploaded: Signal = signals.signal("file-uploaded")


# ── 事件订阅者（独立模块，可单独测试） ──

@order_created.connect
def send_order_confirmation(sender: Any, order: dict[str, Any], **extra: Any) -> None:
    """订单确认邮件 — 订阅者 1"""
    print(f"[Email] Sending confirmation for order {order['id']}")
    # send_email(order["customer_email"], template="order_confirmation", ctx=order)


@order_created.connect
def reserve_inventory(sender: Any, order: dict[str, Any], **extra: Any) -> None:
    """预留库存 — 订阅者 2"""
...
@order_paid.connect
def update_loyalty_points(sender: Any, order: dict[str, Any], **extra: Any) -> None:
    """更新积分 — 订阅者"""
    points: int = int(order.get("total", 0) / 10)
    print(f"[Loyalty] Adding {points} points for order {order['id']}")


@user_registered.connect
def create_default_workspace(sender: Any, user: dict[str, Any], **extra: Any) -> None:
    """创建默认工作空间 — 订阅者"""
    print(f"[Workspace] Creating default workspace for user {user['id']}")


@user_registered.connect
def send_welcome_notification(sender: Any, user: dict[str, Any], **extra: Any) -> None:
    """发送欢迎通知 — 订阅者"""
    print(f"[Notification] Sending welcome message to {user['email']}")


# ── 使用信号（视图中只发送事件，不知道有哪些订阅者） ──
app_signal: Flask = Flask(__name__)


@app_signal.route("/api/orders", methods=["POST"])
def create_order() -> tuple[dict[str, Any], int]:
    ...
    # 发送事件 — 发布者不关心谁在听
    order_created.send(app_signal, order=order)

    return order, 201


@app_signal.route("/api/orders/<int:order_id>/pay", methods=["POST"])
def pay_order(order_id: int) -> dict[str, str]:
    # 处理支付...
    order: dict[str, Any] = {"id": order_id, "total": 150.00}
    order_paid.send(app_signal, order=order)
    return {"status": "paid"}
```
...
| 场景 | 推荐方式 | 原因 |
|------|---------|------|
| 核心业务流程（如库存扣减） | 直接调用 + 事务 | 需要事务一致性和错误处理 |
| 通知/日志/审计 | 信号 | 允许失败不影响主流程 |
| 缓存失效 | 信号 | 多个缓存层独立应对 |
| 第三方集成 | 信号 | 可随时开启/关闭 |
| 性能监控 | 信号 | 不影响业务代码 |
...
# ── 问题：信号同步执行阻塞请求 ──
def slow_subscriber(sender: Any, **kwargs: Any) -> None:
    """耗时订阅者 — 阻塞请求响应"""
    time.sleep(2)  # 模拟耗时的外部调用
    print("Slow subscriber done")
...
def async_subscriber(sender: Any, **kwargs: Any) -> None:
    """异步风格的订阅者"""
    _executor.submit(_slow_work, kwargs)


def _slow_work(kwargs: dict[str, Any]) -> None:
    time.sleep(2)
...
CACHE_TTL: float = 60.0  # 60 秒


def cached_counter_subscriber(sender: Any, **kwargs: Any) -> None:
    """带缓存的订阅者 — 减少外部调用"""
    ...
    _counter_cache["last_updated"] = int(now)
```
...
| 优化措施 | 效果 | 适用场景 |
|---------|------|---------|
| 线程池执行 | 不阻塞请求 | 耗时操作（邮件、HTTP 调用） |
| 本地缓存 | 减少重复计算 | 高频触发的信号 |
| 批处理 | 减少 IO 次数 | 批量日志写入 |
| 消息队列 | 完全解耦 | 生产级异步任务 |
| 条件过滤 | 减少不必要执行 | 有针对性的订阅 |

### 5.9 调试与排错实战

**场景：MethodView 的 POST 请求返回 405 Method Not Allowed**
...
│                                                             │
│  Step 1：检查 URL 注册                                       │
│  app.add_url_rule("/api/users", methods=["GET", "POST"])     │
│  → 确认 methods 列表包含 "POST"                             │
│                                                             │
│  Step 2：检查类中是否定义 post() 方法                        │
│  class UserResource(MethodView):                            │
│  MethodView 自动从类方法推断允许的 HTTP 方法                  │
│  → 确认类中有 def post(self):                               │
│                                                             │
│  Step 3：确认 decorators 不干预                              │
│  decorators = [login_required]                             │
│  → 如果装饰器返回 401/403，不是 405                          │
│                                                             │
│  Step 4：测试实际请求                                         │
...
| 现象 | 可能原因 | 检查方式 |
|------|---------|---------|
| 接收器不执行 | 弱引用被 GC | 使用 `weak=False` 或保存强引用 |
| 接收器不执行 | sender 不匹配 | 检查 `.connect(handler, sender=app)` |
| 接收器不执行 | 信号未被 send | 在信号发送处添加日志 |
| 接收器静默失败 | 异常被吞掉 | Blinker 默认吞异常，加 try/except |
