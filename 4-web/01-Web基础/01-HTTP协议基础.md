...
| 反模式 | 问题 | ✅ 正确做法 | 适用场景 |
|--------|------|-----------|---------|
| **每次请求新建连接** | TCP 握手开销大，延迟显著 | 使用 `requests.Session()` | 需要连续请求同一主机时 |
| **忽略响应状态码** | 错误被静默吞掉，难以定位问题 | 调用 `response.raise_for_status()` | 所有 HTTP 请求 |
| **不设置超时** | 请求可能永久挂起，耗尽连接池 | `requests.get(url, timeout=5)` | 生产环境必设 |
| **用 GET 做修改操作** | 违反 HTTP 语义，浏览器预加载/缓存在危险 | 用 POST/PUT/DELETE | 涉及数据修改的所有端点 |

> **何时使用 Session？** — 与同一主机有 2 次以上的请求时。  
> **何时必须设置超时？** — 所有生产环境请求，防止雪崩效应。
