...
├── app/                      # Source (34/39 projects)
├── tests/                    # pytest (173 test files under tests/)
└── uv.lock                   # If deps installed (27/39 projects)
```

- All 39 projects use hatchling. 34 use `packages = ["app"]`, 1 uses `processor`, 1 uses `analyzer`, 1 uses `src/`, 2 have no explicit package.
- 20/39 projects have `[tool.ruff]` (line-length 88, py313, rules: E,F,I,N,W,UP,B,SIM). Projects without ruff config have no lint setup.
- 5 use `pytest-asyncio` with `asyncio_mode = "auto"`.
- Dev deps: 17 use `[dependency-groups]`, 5 use `[project.optional-dependencies]`. Prefer `[dependency-groups]` with `uv sync --group dev`.
...
**VitePress docs site** (root-level, uses npm). Sidebar auto-generated by `vitepress-sidebar`:
...
```bash
./scripts/verify-code.sh   # Validate all chapter Python code
```
