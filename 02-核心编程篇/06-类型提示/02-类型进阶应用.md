...
│                                                             │
│  • 'T' 是字符串，给类型检查器的名称标识                       │
│  • T 是 Python 变量，代码中用来引用这个类型变量               │
│  • 两者通常保持一致，便于理解和调试                           │
│  • 可以不同：MyType = TypeVar('T') 但不推荐                  │
│                                                             │
│  3. 类型变量何时被替换？                                      │
│  ─────────────────────────────────────────────              │
│  • 定义时不替换：def first(items: list[T]) → T               │
│  • 使用时替换：first([1, 2, 3]) → T 变成 int                 │
│  • 由类型检查器推断，不是运行时替换                           │
│                                                             │
│  4. 常见误解                                                 │
│  ─────────────────────────────────────────────              │
│  ❌ 误解：T 是一种特殊的类型                                  │
│  ✅ 正解：T 是占位符，表示"某种类型，使用时确定"              │
│                                                             │
│  ❌ 误解：TypeVar 创建了新的数据类型                          │
│  ✅ 正解：TypeVar 只是声明一个类型变量名                      │
│                                                             │
│  ❌ 误解：运行时 T 会变成具体类型                             │
│  ✅ 正解：运行时 T 只是标记，替换发生在静态分析时             │
│                                                             │
│  5. 类比理解                                                 │
│  ─────────────────────────────────────────────              │
│  函数参数：def f(x): ...     x 是变量，调用时用具体值替换    │
│  类型变量：def f(items: list[T]): ...  T 是类型占位符       │
│                                                             │
│  比喻：                                                     │
│  • TypeVar('T') = 声明"有个类型位置，暂用 T 表示"           │
│  • list[T]      = "某个类型的列表，具体类型待定"            │
