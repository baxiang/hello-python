...
> **引入版本**：Python 3.0+（基础魔术方法）；Python 3.0+（容器协议）
> **贯穿项目**：oop_demo/
...
| 做法 | 原因 | 示例 |
|------|------|------|
| **同时定义 `__eq__` 和 `__hash__`** | 相等对象必须有相同哈希 | `def __hash__(self): return hash(self.member_id)` |
| **`__repr__` 包含类名和关键字段** | 调试时能快速识别对象 | `return f"Member({self.name!r}, {self.email!r})"` |
| **`__eq__` 返回 `NotImplemented`** | 让 Python 尝试反向比较 | `if not isinstance(other, Member): return NotImplemented` |
| **容器类实现 `__len__` + `__iter__`** | 支持 `len()` 和 `for in` | `def __len__(self): return len(self._books)` |
